...
MPU6500_WE::MPU6500_WE(TwoWire *w, int addr)
    : _wire(w), i2cAddress(addr)
{
    // intentionally empty
}

MPU6500_WE::MPU6500_WE(SPIClass *s, int cs, bool spi)
    : _spi(s), csPin(cs), useSPI(spi)
{
    // intentionally empty
}
...
void MPU6500_WE::autoOffsets()
{
    ...
    //writeMPU9250Register(REGISTER_ACCEL_CONFIG, 0xE0);
    //writeMPU9250Register(REGISTER_GYRO_CONFIG, 0xE0);
    ...
    //writeMPU9250Register(REGISTER_ACCEL_CONFIG, 0x00);
    //writeMPU9250Register(REGISTER_GYRO_CONFIG, 0x00);

    // acceleration
    accelerationOffsetAccumulator /= 100.f;
    ...
    //if (abs(accelerationOffsetAccumulator.y) == bottom)
    //    accelerationOffsetAccumulator.y -= (16384.0f * (signbit(accelerationOffsetAccumulator.y) ? 1 : -1));
    //if (abs(accelerationOffsetAccumulator.z) == bottom)
        accelerationOffsetAccumulator.z -= 16384.0f * sgn(accelerationOffsetAccumulator.z);

    //accelerationOffsetAccumulator.z -= 16384.0f;

    accOffsetVal = accelerationOffsetAccumulator;
    // gyro
    gyrOffsetVal = gyroOffsetAccumulator / 100.f;
}
